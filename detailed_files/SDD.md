...
> **Companion to:** PRD v1.0 (frozen) · **Structure:** per frozen ToC (`SDD.md`, v2.1)
> **Status:** Authoring — Part A (Chapters 1–3) complete

**Canonical terminology (used verbatim throughout — no synonyms).** *Agent* · *Governance Request* (the proposed action an Agent submits) · *Decision* (allow / deny / escalate) · *Policy* · *Risk Evaluation* · *Confidence* · *Explainability* · *Approval* · *Evidence* · *Decision Engine* · *Integration Layer (PEP)* · *Decision Plane* / *Control Plane*.
...
**Why this diagram exists.** It illustrates the single most important structural decision: the split between a **Decision Plane** on the critical path (kept minimal, fast, and fail-safe) and a **Control Plane** off it (registry, policy authoring, containment, query). Containment reaches the Decision Plane *out-of-band* (dashed) so an Agent can be stopped even if the Control Plane is degraded — the architectural expression of P9.
...
| D7 | **Runtime authority evaluation** — no cached *allow*; entitlements and policy resolved per request | An Agent's authority is decided at the moment of action (P3) | Ch 9, Ch 10 |
...
The system is a two-plane architecture — a minimal, fail-safe **Decision Plane** on the critical path and a **Control Plane** beside it — connected to an append-only **Evidence Ledger**, driven by an inspectable **AI Decision Pipeline**, and bounded strictly to *deciding* rather than *executing*. The remaining chapters develop each of decisions D1–D8.

> **Chapter self-review.** No PRD duplication (business motivation referenced by ID only). Terminology canonical. Depth appropriate for orientation; detail deferred to named chapters. Answers its stated purpose (whole-system orientation). Implementation-oriented. ✔
...
The automated (non-escalated) path must add ≤ [50 ms] P95. The budget allocates that ceiling across the in-path steps; anything requiring a synchronous external call or human judgment is, by design, kept *off* this path.
...
**Trade-off.** Keeping the external Fraud signal off the synchronous path (consumed as a cached/asynchronous input) is what makes the budget achievable; the cost is that a *stale-signal* window exists, which the fail-safe rule (D6) and Risk-Evaluation design (Ch 9) explicitly account for.

### 2.4 Architecture principles

These are *engineering* principles — distinct from the PRD's product principles (P1–P10), which they operationalize. Each names the product principle it enforces.
...
| **AP-3** | Authority (policy + entitlement) is resolved at request time; no cached *allow* | P3 | Revocation and policy change take effect on the next request |
...
The system's qualities are engineered, not assumed: a scenario-based quality model, a concrete latency budget that keeps external and human steps off the synchronous path, and ten architecture principles (AP-1–AP-10) that bind every later decision to a product principle. Fail-safety, reproducibility, and neutrality are the non-negotiables the rest of the design must never violate.

> **Chapter self-review.** No duplication with Ch 1 (Ch 1 = orientation; Ch 2 = targets/principles). Quality targets reference NFR IDs, not restated prose. Terminology canonical. Latency budget is system-specific, not textbook. Answers purpose (the yardstick). Implementation-oriented. ✔
...
This chapter is the bridge from the frozen PRD to this architecture. It demonstrates that every architectural element exists to satisfy a stated requirement — nothing is arbitrary — and that no requirement is left without a home. It is a *mapping*, not a restatement; the full cross-reference is Appendix A.
...
Every capability maps to a subsystem, every functional-requirement group to a component, and every non-functional requirement to a concrete mechanism — with no orphans in either direction (verified in Appendix A). This traceability is what lets the ARB confirm the architecture is *complete and non-arbitrary* before reviewing a single component in depth.

> **Chapter self-review.** Pure mapping — no PRD content restated, no overlap with Ch 1–2. Terminology and IDs consistent with PRD and Ch 1–2. Appropriately concise (a bridge, not a chapter of prose). Answers its purpose (completeness/traceability). ✔
...
*Part B presents the system through multiple architectural views (per the frozen ToC). It stays at the level of structure, responsibility, and interaction — internal algorithms, data internals, API payloads, and component internals are intentionally reserved for Part C.*
...
> **Key insight.** The Agent — not the platform — calls Core Banking, and only after an *allow*. The platform is never in the money-movement path itself; it is in the *decision* path. This is what lets it be fail-safe without being able to cause a settlement error.
> **Trade-off.** Because execution stays with the Agent, the platform cannot *guarantee* an allowed action executes exactly as proposed; it guarantees only that an un-allowed action should not proceed. That residual is accepted and revisited in the threat model (Ch 13).
...
### 4.8 Chapter summary & self-review

The platform is a single black box across four trust zones, consuming identity and risk signals, emitting Decisions, and never executing. This boundary is the foundation for integration and security.

> **Self-review.** HLD altitude held — no internals, payloads, or data structures. Terminology canonical. No PRD restatement (scope referenced by ID). Answers its purpose (boundary + trust zones). ✔
...
> **Key insight.** The Control Plane feeds the Decision Plane *asynchronously* (policy bundles, entitlements are pushed/cached), so no Control-Plane component is a synchronous critical-path dependency. This is what makes NFR-1.1 availability attainable — the decision path can keep deciding even if policy authoring or the dashboard is down.
> **Trade-off.** Pushing policy/entitlements to the Decision Plane introduces a propagation delay (bounded, and required to be effective on the next decision per FR-2.3); the design accepts eventual-consistency of *distribution* while keeping *evaluation* strictly at request time (AP-3).
...
| Pure asynchronous (observe-and-veto) | Cannot *prevent* a high-risk action before it executes; fails the core preventive promise (P8) |
...
### 5.8 Chapter summary & self-review

The logical architecture is a two-plane model: a fast, stateless, fail-safe Decision Plane and a rich, latency-tolerant Control Plane, joined asynchronously through an event backbone and an Evidence Ledger, with enforcement mode chosen by risk tier.

> **Self-review.** No implementation, DB, payload, or internal-algorithm detail (path *selection* shown, not the classification algorithm — that is Ch 9). Terminology canonical. Alternatives and trade-offs explicit. Answers its purpose (the logical model + central pattern). ✔
...
Per the writing standard, each major subsystem is characterized by its cross-cutting behavior (developed fully in Parts C–D; summarized here at HLD level).
...
### 6.6 Chapter summary & self-review

Nine components across two planes, each with a single responsibility and an explicit non-responsibility, mapped to F-1–F-9. The Decision Plane is deliberately minimal; cross-cutting qualities are summarized and deferred to Parts C–D.

> **Self-review.** Component *responsibilities and boundaries* only — no internal module structure, interfaces-by-payload, or algorithms (reserved for Part C, Ch 9–10). Feature mapping consistent with PRD. Terminology canonical. ✔
...
This chapter specifies *how* Agents connect to governance and how the platform integrates with external systems — at the architectural level. Because adoption is a top PRD risk (R-1) and the "one front door" is F-1, the integration pattern is a first-class architectural concern, not a detail.
...
> **Key insight.** All three patterns speak the *same* governance contract to the Decision Engine; the pattern is an Agent-side deployment choice, not a different governance model. This is what keeps governance uniform (NFR-10.1) while meeting Agents where they run.
...
> **Design note.** The asymmetry is deliberate: loss of *identity* fails everything closed (an unidentifiable request cannot be governed), while loss of a *risk signal* fails only high-risk closed and lets low-risk proceed on internal criteria. This matches the proportionality principle (AP-4) rather than treating all dependency loss identically.
...
### 7.6 Chapter summary & self-review

Three interchangeable PEP deployment patterns share one governance contract, and external integrations degrade fail-safe with a deliberate identity-vs-signal asymmetry. Integration is engineered for adoption and safety, not convenience.

> **Self-review.** Patterns and contracts named, not their payloads or protocols (Part C). Failure behavior described at boundary level. Terminology canonical. Addresses adoption risk R-1 by ID without restating it. ✔
...
This chapter shows how the components collaborate *over time* for the critical flows, and the lifecycle states the key entities move through. It is where the static structure of Chapters 5–7 becomes observable behavior — including, explicitly, the fail-safe path.
...
> **Key insight.** The Agent blocks until the Decision resolves — including across a human Approval — which is exactly what "preventive governance" requires for high-risk actions. Evidence is written on *every* branch, not just the terminal one, preserving reproducibility of escalations.
...
> **Key insight.** Governance still happens and is still recorded; only the *blocking* is removed. This keeps low-risk latency minimal while preserving the complete Evidence record (NFR-5.2).
...
> **Key insight.** *Registered* is not *Active*: an Agent that lacks an owner or entitlements cannot obtain an allow — the state model enforces "no ownerless decision" structurally, not by policy alone.
...
> **Key insight.** *Escalated* is an explicit state with its own recorded transitions, not a side-effect — which is why an escalation can be reconstructed and explained as faithfully as an automated allow/deny.
...
### 8.8 Chapter summary & self-review

The critical flows — high-risk-with-escalation, low-risk-async, and containment — and the Agent and Decision state machines make the system's behavior observable, with the fail-safe and escalation paths drawn explicitly and Evidence written on every branch.

> **Self-review.** Interactions and states only — no internal algorithms (how confidence is computed is Ch 9), no payloads, no data internals. Terminology canonical. Fail-safe path explicit. Answers its purpose (runtime behavior). ✔
...
> **Key insight.** Explainability is generated *before* composition and independently of the outcome, so an explanation exists for allow, deny, and escalate alike — explainability cannot be skipped for "simple" allows.
...
> **Trade-off.** A shared context is a mutable object passed through the chain; the design constrains each stage to *append* to it (never overwrite prior stages' results), preserving reproducibility.
...
| Explainability Generation | Produce a human-readable reason (AP-6) | Cannot explain → the outcome cannot be *allow* on high-risk |
...
*(How models are trained, tuned, or evaluated is out of scope for the SDD — this is an architectural boundary, not a modeling document.)*
...
### 9.9 Chapter summary & LLD review

The Decision Pipeline is a fail-fast Pipeline/Chain-of-Responsibility of seven stateless stages sharing a request-scoped Decision Context, with model-backed logic isolated behind Strategy interfaces and structurally barred from holding decision authority.

> **LLD review.** Engineering design, not code — no classes, language, or framework. Model *internals* excluded by design. Patterns justified against AP-x. Consistent with Ch 6 boundaries (this is the Decision Engine's interior) and D/AP. Prepares Ch 13 (security of model boundary) and Ch 14 (fail-safe/scale). ✔
...
> **Key insight.** Distribution is asynchronous but *evaluation* binds to a specific version at decision time; a decision can therefore always be replayed against the exact version that governed it.
...
| **Patterns** | **Event Sourcing** (the ledger *is* the source of truth), **Outbox** (reliable decision→ledger hand-off), **Append-only Log / WORM** |
...
| Failure behavior | Designed to *stop*, so its failure mode biases toward containment, not release |
...
### 10.10 Chapter summary & LLD review

The seven components are each engineered around recognizable patterns chosen to serve the architecture principles: Event Sourcing + Outbox for tamper-evident, loss-free Evidence; State Machine + Saga for durable escalation; CQRS for decoupled visibility; Repository + Temporal for entitlements; and out-of-band control for containment.

> **LLD review.** Engineering-level, implementation-agnostic — no framework or language. Patterns justified, not decorative. Consistent with Ch 6 boundaries and D/AP. Naturally sets up the cross-cutting concerns (reliability, security) of Part D. ✔
...
*Architecture-level; representative attributes only.*
...
> **LLD review.** Data *architecture* — structure, ownership, keys, indexing, partitioning, versioning, lifecycle — with no SQL, storage-engine names, or field-level exhaustion. Consistent with D5/AP-5 and Ch 10 Evidence design. Prepares Ch 13 (data protection) and Ch 14 (partitioned scale/DR). ✔
...
*Semantic (not schema) shape, to illustrate the fail-safe contract:*
...
> **Key insight.** The contract has no "undefined failure" state for a high-risk request: even a platform error resolves to a safe, explained Decision. This makes fail-safety a property of the *interface*, not just the implementation.
...
> **LLD review.** Contracts specified at the interface/semantic level with representative shapes — no framework, transport, or serialization specifics. Fail-safe and idempotency are contract properties, consistent with AP-8 and the Evidence design. Prepares Ch 13 (surface authZ) and Ch 15 (contract SLOs). ✔
...
*Part D answers one question: how does this architecture **survive and operate in production**? It reads as a production-readiness review — security, resilience, and operations examined as architectural properties of this specific system, not as generic practice. NFRs are the targets; this Part shows the mechanisms that meet them.*
...
| Platform → Fraud signals | Signals are advisory inputs, possibly stale/absent | A signal can never *raise* trust enough to bypass policy; its absence fails high-risk safe (AP-8) |
| Platform → Execution | Downstream and outside platform control | The platform's guarantee ends at the Decision; it secures the *decision*, not the settlement |
...
> **Key insight.** Internal components never re-derive identity from Agent-supplied data; they consume the *verified* principal bound at the PEP. This removes an entire class of internal spoofing — an internal component cannot be tricked into trusting a forged identity because it never parses one.
...
| Approval | Ensure only authorized, *distinct* approvers resolve | Cannot originate a Decision |
...
*The full STRIDE analysis, with per-asset detail, is Appendix C.*

### 13.10 Architectural Decisions Realized & self-review
...
> **Self-review.** Architecture-level — no security products, protocols, or algorithms named. Boundaries, trust assumptions, per-subsystem responsibilities, secrets philosophy, least privilege, defense-in-depth, secure-comms assumptions, and identity propagation all covered. Consistent with D/AP and Ch 4 zones. ✔
...
> **Design insight.** Read the "safe degradation" column top to bottom: every failure resolves toward *blocking high-risk actions and preserving Evidence*, never toward permitting them. This is AP-8 expressed as an operational property — the system's worst day still produces safe, recorded outcomes.
...
### 14.7 Architectural Decisions Realized & self-review
...
> **Self-review.** Structured around failure, not generic HA/DR. Every failure has detection/immediate/degradation/recovery/impact. System-specific (Evidence, Policy distribution, Approval), not textbook. Consistent with AP-8/AP-2/D5. ✔
...
### 15.10 Architectural Decisions Realized & self-review
...
> **Self-review.** Deployment/operations treated as architecture, not vendor infra — no cloud specifics. Operational Observability vs Business Evidence explicitly separated with rationale. Policy-as-config, shadow rollout, and immutable rollback are system-specific, not generic DevOps. Consistent with D/AP and PRD §6. ✔
...
*Part E is the engineering justification for the architecture in Parts A–D. Chapter 16 evaluates the technology class for each architectural element as a design review; Chapter 17 formalizes — without re-deriving — the decisions the document already made; Chapter 18 shows the architecture's extensibility against concrete seams. No new architecture is introduced.*

## Chapter 16 — Technology Selection & Rationale

### 16.1 Approach

This chapter selects the **class of technology** for each architectural element on properties, not product names — vendors are named only where a class is best identified by an exemplar. Each element is the persistence or platform choice for an already-named subsystem, evaluated in a consistent format: *architectural requirement → evaluation criteria → candidate approaches → trade-off analysis → selection → why it best satisfies Chapters 2–15*.
...
- **Trade-off analysis.** Embedded-in-code violates policy-as-config (FR-2.x, Ch 15.4); a BPM platform adds latency and operational weight; a declarative in-process engine gives fast evaluation *and* externalized, versionable policy.
...
### 16.11 Selection summary & self-review
...
> **Self-review.** Every element evaluated as a design review on properties, not a product list; classes named only for identification. Each selection is anchored to earlier decisions and quality attributes. No new architecture introduced. ✔

---

## Chapter 17 — Architecture Decision Records (ADRs)

*These ADRs formalize decisions already made in Parts A–D. They explain **why** the architecture looks as it does; they introduce nothing new. Format: Context · Problem · Decision · Alternatives · Consequences · Relationship to D/AP.*
...
Context: entitlements and policy change over time. Problem: pre-granted permissions go stale. Decision: evaluate policy + entitlement at request time; never cache an *allow* decision. Alternatives: cached decisions (rejected — stale authority). Consequences: revocation/policy changes effective on next request; per-request cost. Realizes **D7**, supports **AP-3**.
...
> **Self-review.** Every ADR formalizes an existing decision, cites its D/AP, and introduces no new architecture. Structure is consistent across all fourteen. ✔

---

## Chapter 18 — Future Technical Evolution

*Each evolution is expressed against a concrete existing seam — extension point, required change, what stays fixed, and why the architecture supports it — not as roadmap aspiration.*
...
> **Self-review.** Every evolution names a real seam introduced earlier and states what stays fixed. No generic roadmap items; each is an extensibility argument grounded in the existing architecture (NFR-9.x). ✔
...
*Full FR→component and NFR→mechanism traceability (summarized inline in Ch 3).*
...
# Architecture Review Board — whole-document consistency pass

A final review of the complete SDD (Chapters 1–18 + appendices) against the ARB checklist.

| Check | Verdict | Evidence |
|-------|---------|----------|
| No duplicated concepts | ✔ | Each chapter has a unique purpose; HLD structure vs LLD internals never repeat |
| No contradictory statements | ✔ | Fail-safe, neutrality, and plane-separation stated consistently from Ch 1 through Ch 18 |
| Consistent terminology | ✔ (1 harmonized) | Canonical terms fixed in front matter; **"Evidence Store" reconciled as the persistence for the "Evidence Ledger"** (Ch 16.4) to avoid a synonym |
| Consistent use of D1–D8 | ✔ | Defined Ch 1.4; realized-tables in every chapter; formalized in ADR-1–ADR-8/10 |
| Consistent use of AP-1–AP-10 | ✔ | Defined Ch 2.4; cited throughout; each ADR maps to them |
| Every chapter supports Ch 2 quality attributes | ✔ | Ch 5.5, 14, 15.9 map directly; others cite NFR targets |
| Every architectural decision justified | ✔ | Trade-offs in-line (Parts B–D) + consolidated in ADRs (Ch 17) + tech rationale (Ch 16) |
| Every diagram has a purpose | ✔ | All 20+ diagrams carry Purpose/Decisions/Insight/Trade-off |
| Every subsystem has clear boundaries | ✔ | Ch 6 responsibilities + explicit non-responsibilities; Ch 9–10 records |
| Every Ch 3 requirement addressed later | ✔ | CAP/FR/NFR mappings realized in Ch 5–15; Appendix A cross-references |
| HLD/LLD boundary preserved | ✔ | Part B: no internals/payloads/DB; Part C: internals; deferrals flagged where crossed |
| Security/resilience/deploy/ops align with earlier architecture | ✔ | Part D builds only on the two-plane model, Evidence design, and AP set |

**One consistency correction applied:** the technology element in Ch 16.4 is titled **"Evidence Store (persistence for the Evidence Ledger)"** so the document uses one name — *Evidence Ledger* is the subsystem, *Evidence Store* its persistence — rather than two terms for one concept. No other inconsistencies found.

**ARB verdict.** The SDD reads as one coherent enterprise architecture document. Every architectural decision (D1–D8), principle (AP-1–AP-10), and requirement (CAP/FR/NFR) is introduced once, justified, and traced end-to-end; the HLD/LLD boundary holds; and the cross-cutting chapters extend — never contradict — the core architecture. **Recommended for approval.**

---

*End of Solution & System Design Document. Chapters 1–18 + Appendices A–D complete. The sole external reference is PRD v1.0. This document, with the PRD, is the input to Deliverable 3 — the Technical Architecture Presentation.*
